...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    # )
    ret.vEgoRaw = 24 # cp_PT.vl['Speed']["VehicleSpeed"] * CV.KPH_TO_MS
    ...
    ret.cruiseState.available = True # not ret.espDisabled  #cruise not available when DSC fully off
    ...
    # ret.cruiseState.enabled = True #cp_PT.vl["DynamicCruiseControlStatus"]['CruiseActive'] != 0
    ...
    self.cruise_stalk_cancel = int(cp_aux.vl['STEERING_STATUS']['DEBUG_STATES']) & 0x2 != 0 # break enabled state
  ...
  def update_button_enable(self, buttonEvents: list[structs.CarState.ButtonEvent]):
    return not self.cruise_stalk_cancel
